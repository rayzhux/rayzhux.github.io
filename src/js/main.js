(function () {
  ...
  // 提交邮箱
  var elemSubmit = document.getElementById('submit-email');
  var inputElem = document.getElementById('input-email');
  var submiting = false;
  elemSubmit.onclick = function(){
    if(submiting) return false;

    if(!/button-shadow__disable/i.test(elemSubmit.className)){
      if(!inputElem.value){
        inputElem.focus();
      ...
      submiting = true;
      submitEmail(inputElem.value).then(() => {
        submiting = false;
        inputElem.value = '';
        elemSubmit.className = elemSubmit.className + ' button-shadow__disable';
  ...
  function submitEmail(email){
    const ERR_MSG = 'Submission failed, please try again.';
      console.log({
        email
      });
      return new Promise((resolve, reject) => {
        fetch(window.HOST_API_PATH,{
          method: 'post',
            headers:{
          ...
          body: JSON.stringify({
            query: `query{
              createOrUpdateContact(email:"${email}")
            }`,
          }),
      ...
      if(Response.errors && Response.errors.length){
        ...
        reject(Response.errors[0].message || ERR_MSG);
      } else {
